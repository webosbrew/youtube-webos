...
function createOptionsPanel() {
  ...
  version.textContent = `v${__YTAF_VERSION__ || '.X'}`;
...
async function initAudioOnlyToggle() {
  ...
  const WATCH_CONTROLS_SELECTOR = '.ytLrWatchDefaultControls';
  ...
  const updateOverlay = (root = document) => {
    const controls = root.querySelector(WATCH_CONTROLS_SELECTOR);
    if (!controls) return;

    let overlay = controls.querySelector(`.${YTAF_OVERLAY_CLASS}`);
    if (audioOnlyEnabled) {
      if (!overlay) {
        overlay = Object.assign(document.createElement('div'), {
          textContent: 'Audio Mode Enabled - Press [BLUE] to toggle',
          className: YTAF_OVERLAY_CLASS
        });
        controls.prepend(overlay);
      }
    } else {
      overlay?.remove();
    }
